...
while chos~=possibility,
    chos=menu('Fingerprint Recognition System','Encrypt image and add to database','Select image for fingerprint recognition','Info','Delete database',...
        'Fingerprint image: visualization','Gabor Filter: visualization','Exit', 'Encrypt and store fingerprint');
    %--------------------------------------------------------------------------
    %--------------------------------------------------------------------------
    %--------------------------------------------------------------------------
    % Calculate FingerCode and encrypt and Add to Database
    if chos==1
        clc;
        close all;
        selezionato=0;
...
%             data{fp_number,2}=finger_code2;
            ...
            for (i=1:1:num_disk)
                thisfc = finger_code1(i);
                for (sector=1:1:n_sectors)
                    data{fp_number,1,i,sector} = homEncrypt(int32(thisfc{1}(sector)));
                end
            end
            
            for (i=1:1:num_disk)
                thisfc = finger_code2(i);
                for (sector=1:1:n_sectors)
                    data{fp_number,2,i,sector} = homEncrypt(int32(thisfc{1}(sector)));
                end
            end
